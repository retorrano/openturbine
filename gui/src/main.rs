...
fn create_turbine_blade(blade_length: f32) -> Mesh {
    ...
    for i in 0..=num_sections {
        ...
        for j in 0..=num_points {
            ...
            // Swap Y and Z so the blade extends along the Y axis (radially from the hub)
            ...
            // Swap Y and Z for normals as well
...
fn physics_system(mut state: ResMut<SimulationState>) {
    ...
    // Smoothly accumulate data for real-time charts in the GUI
    ...
    // High-resolution accumulation (0.01 m/s threshold) for ultra-smooth lines
...
fn apply_theme(ctx: &egui::Context, theme: Theme) {
    ...
    // Explicitly set high-contrast text colors and panel fills
    match theme {
        Theme::Dark => {
                ...
                if ui.button("Export Results...").clicked() {
                    ...
                    if let Some(path) = files {
                        ...
                        if let Some(mut f) = writer {
                            ...
                            // Export the actual recorded history to reflect the GUI state
    ...
    if state.show_parameters {
        egui::SidePanel::left("parameters")
            ...
            if ui.button("Refresh Power Curve").clicked() {
                ...
                // High density sweep (0.1 m/s steps) for ultra-smooth UI graphs
                let mut ws = 0.0;
                while ws <= 30.0 {
...
fn export_pdf_report(
    ...
    y_pos -= 10.0;
    current_layer.use_text(format!("Version: 0.1.1 | Date: April 19, 2026"), 12.0, Mm(20.0), Mm(y_pos), &font);
    y_pos -= 15.0;
    ...
    // Results Table - now using the pre-calculated consistent data from the session
    ...
    // Use the actual historical data for the table, sampling it to avoid overcrowding
    // We sample up to 25 rows to fit on the first page
    let sample_count = 25;
    let step = (power_data.len() / sample_count).max(1);
    for i in (0..power_data.len()).step_by(step) {
        if y_pos < 50.0 { break; } // Don't overwrite citation
    ...
    // Citation at the bottom of Page 1 - Wrapped to avoid overflow
    ...
    current_layer.use_text("(Version 0.1.1). Available at: https://github.com/openturbine/openturbine-rs", 9.0, Mm(20.0), Mm(25.0), &font);
    ...
    // Helper to generate a plot into a buffer
    let render_plot = |title: &str, label_y: &str, data: &Vec<(f64, f64)>, color: &RGBColor| -> Vec<u8> {
        let img_width = 2400; // High resolution
        let img_height = 2000; // Maximize height for the long format
    ...
    for (title, label_y, data, color) in plots {
        ...
        // Scale 1.0 at 300 DPI = 8 inches width on 8.5 inch page.
        // We maximize scaling to fill more of the page.
        image.add_to_layer(layer_ref.clone(), ImageTransform {
            translate_x: Some(Mm(6.35)), // Centered
            translate_y: Some(Mm(40.0)),  // Centered vertically
            scale_x: Some(1.0),
            scale_y: Some(1.0),
            ..Default::default()
        });
