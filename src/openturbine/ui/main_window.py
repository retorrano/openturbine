...
try:
    from PySide6.QtWidgets import (
        ...
        QSplitter, QTreeWidget, QTreeWidgetItem, QTabWidget, QStatusBar,
        QMenuBar, QMenu, QToolBar, QDockWidget, QLabel, QPushButton,
        QGroupBox, QFormLayout, QLineEdit, QSpinBox, QDoubleSpinBox,
        QComboBox, QCheckBox, QSlider, QProgressBar, QTextEdit,
        QTableWidget, QTableWidgetItem, QHeaderView, QFileDialog,
        QMessageBox, QDialog, QSizePolicy, QFrame
    )
...
class MainWindow:
    ...
    def _update_power_display(self):
        ...
        self.status_label.setText(f"Power: {result['power_mw']:.5f} MW, RPM: {result['rotor_rpm']:.5f}")
    ...
    def _draw_turbine_schematic(self, wind_speed, blade_angle):
        ...
        for i in range(self.simulator.num_blades):
            ...
            self.ax_2d.plot(left_x + [tip_x] + right_x[::-1], 
                           left_y + [tip_y] + right_y[::-1], 
                           'k-', linewidth=1)
        
        rotor_circle = plt.Circle((hub_x, hub_y), blade_len, fill=False, 
        ...
        for i in range(-3, 4):
            ...
            self.ax_2d.annotate('', xy=(x_pos + speed_factor, hub_y), 
                               xytext=(x_pos, hub_y),
        ...
        if is_running:
            self.ax_2d.set_title(f'Wind Turbine - {wind_speed:.0f} m/s | Power: {anim_params["power_mw"]:.2f} MW', 
                                 color='white', fontsize=12, pad=10)
        else:
            self.ax_2d.set_title(f'TURBINE STOPPED (wind > 25 m/s cut-out)', 
                                 color='yellow', fontsize=12, pad=10)
    ...
    def _create_view_controls_dock(self):
        ...
        self.opt_ground.toggled.connect(lambda c: self._on_ground_toggle(c) if hasattr(self, 'ground_actor') else None)
    ...
    def _on_wind_speed_changed(self, value):
        ...
        self.status_label.setText(f"Wind: {value} m/s | Power: {result['power_mw']:.5f} MW | RPM: {result['rotor_rpm']:.5f}")
