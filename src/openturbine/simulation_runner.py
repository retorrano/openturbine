...
class SimulationRunner:
    ...
    def calculate_power(self, wind_speed: float, pitch_angle: float = 0.0,
                       rotor_rpm: Optional[float] = None) -> float:
    ...
    def run_steady_state(self, wind_speed: float) -> Dict[str, Any]:
        ...
        tsr = (rotor_rpm * 2 * math.pi / 60.0) * (self.rotor_diameter / 2.0) / wind_speed if wind_speed > 0 else 0
        cp = self._calculate_cp(tsr, pitch_angle)
    ...
    def run_time_domain(self, duration: float, dt: float, 
                       wind_speed: Optional[float] = None,
                       turbulence: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            "capacity_factor": np.mean(power_history) / (self.rated_power * duration) * duration if duration > 0 else 0
        }
    
    def estimate_annual_energy(self, mean_wind_speed: Optional[float] = None,
    ...
    def get_blade_tip_position(self, blade_index: int, time: float, 
                              rotor_rpm: Optional[float] = None) -> Tuple[float, float]:
